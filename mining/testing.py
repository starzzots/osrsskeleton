def findobjat(objsRGBVal,coords=(0,0,1080,670),step=1):#topleftx, toplefty, bottomrightx, bottomrighty
    flag=0
    wincap = WindowCapture('RuneLite')
    screenshot=pg.screenshot('test.png',region=(wincap.coords[0],wincap.coords[1],coords[2],coords[3]))
    
    
    for y in range(coords[1], coords[3], step):#1,3
        for x in range(coords[0], coords[2], step):#0,2
